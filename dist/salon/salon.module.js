"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.SalonModule = void 0;
const common_1 = require("@nestjs/common");
const salon_controller_1 = require("./salon.controller");
const salon_service_1 = require("./salon.service");
const typeorm_1 = require("@nestjs/typeorm");
const SalonEntity_1 = require("../TypeOrms/SalonEntity");
const ReviewEntity_1 = require("../TypeOrms/ReviewEntity");
let SalonModule = exports.SalonModule = class SalonModule {
};
exports.SalonModule = SalonModule = __decorate([
    (0, common_1.Module)({
        imports: [typeorm_1.TypeOrmModule.forFeature([SalonEntity_1.SalonEntity, ReviewEntity_1.ReviewEntity])],
        controllers: [salon_controller_1.SalonController],
        providers: [salon_service_1.SalonService],
        exports: [salon_service_1.SalonService]
    })
], SalonModule);
//# sourceMappingURL=salon.module.js.map